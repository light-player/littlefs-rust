...
use lp_littlefs_core::{Lfs, LfsConfig, LfsInfo, LFS_ERR_IO};
...
impl<S: Storage> Filesystem<S> {
    ...
    pub fn format(storage: &mut S, config: &Config) -> Result<(), Error> {
        ...
        let rc =
            lp_littlefs_core::lfs_format(inner.lfs.as_mut_ptr(), &inner.config as *const LfsConfig);
        from_lfs_result(rc)
    }
    ...
    pub fn mount(storage: S, config: Config) -> Result<Self, Error> {
        ...
        let rc =
            lp_littlefs_core::lfs_mount(inner.lfs.as_mut_ptr(), &inner.config as *const LfsConfig);
        if rc != 0 {
    ...
    pub fn unmount(self) -> Result<S, Error> {
        ...
        let rc = if inner.mounted {
            inner.mounted = false;
            lp_littlefs_core::lfs_unmount(inner.lfs.as_mut_ptr())
        } else {
            0
        };
        drop(inner);
