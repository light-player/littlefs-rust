...
use lp_littlefs_core::{LfsFile, LfsFileConfig};
...
impl<'a, S: Storage> File<'a, S> {
    ...
    pub fn write(&self, data: &[u8]) -> Result<u32, Error> {
        let mut inner = self.fs.inner.borrow_mut();
        let rc = lp_littlefs_core::lfs_file_write(
            inner.lfs.as_mut_ptr(),
            self.alloc.file.as_ptr() as *mut LfsFile,
    ...
    pub fn seek(&self, pos: SeekFrom) -> Result<u32, Error> {
        let (off, whence) = match pos {
            SeekFrom::Start(n) => (
                n as i32,
                lp_littlefs_core::lfs_type::lfs_whence_flags::LFS_SEEK_SET,
            ),
            SeekFrom::Current(n) => (
                n,
                lp_littlefs_core::lfs_type::lfs_whence_flags::LFS_SEEK_CUR,
            ),
            SeekFrom::End(n) => (
                n,
                lp_littlefs_core::lfs_type::lfs_whence_flags::LFS_SEEK_END,
            ),
        };
        let mut inner = self.fs.inner.borrow_mut();
        let rc = lp_littlefs_core::lfs_file_seek(
            inner.lfs.as_mut_ptr(),
            self.alloc.file.as_ptr() as *mut LfsFile,
            off,
            whence,
    ...
    pub fn tell(&self) -> u32 {
        let mut inner = self.fs.inner.borrow_mut();
        let rc = lp_littlefs_core::lfs_file_tell(
            inner.lfs.as_mut_ptr(),
            self.alloc.file.as_ptr() as *mut LfsFile,
        );
        drop(inner);
    ...
    pub fn size(&self) -> u32 {
        let mut inner = self.fs.inner.borrow_mut();
        let rc = lp_littlefs_core::lfs_file_size(
            inner.lfs.as_mut_ptr(),
            self.alloc.file.as_ptr() as *mut LfsFile,
        );
        drop(inner);
    ...
    pub fn sync(&self) -> Result<(), Error> {
        let mut inner = self.fs.inner.borrow_mut();
        let rc = lp_littlefs_core::lfs_file_sync(
            inner.lfs.as_mut_ptr(),
            self.alloc.file.as_ptr() as *mut LfsFile,
        );
        drop(inner);
    ...
    pub fn truncate(&self, size: u32) -> Result<(), Error> {
        let mut inner = self.fs.inner.borrow_mut();
        let rc = lp_littlefs_core::lfs_file_truncate(
            inner.lfs.as_mut_ptr(),
            self.alloc.file.as_ptr() as *mut LfsFile,
            size,
        );
