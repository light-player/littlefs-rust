...
use common::{
    assert_ok, default_config, fs_with_hello, init_context, path_bytes, LFS_O_APPEND, LFS_O_CREAT,
    LFS_O_EXCL, LFS_O_RDONLY, LFS_O_TRUNC, LFS_O_WRONLY,
};
use lp_littlefs::{
...
fn test_files_append() {
    ...
    let _ = lfs_file_write(
        lfs.as_mut_ptr(),
        file.as_mut_ptr(),
        part1.as_ptr() as *const core::ffi::c_void,
        part1.len() as u32,
    );
    assert_ok(lfs_file_close(lfs.as_mut_ptr(), file.as_mut_ptr()));
    ...
    let _ = lfs_file_write(
        lfs.as_mut_ptr(),
        file.as_mut_ptr(),
        part2.as_ptr() as *const core::ffi::c_void,
        part2.len() as u32,
    );
    assert_ok(lfs_file_close(lfs.as_mut_ptr(), file.as_mut_ptr()));
    ...
    assert_eq!(n, (part1.len() + part2.len()) as i32);
...
fn test_files_truncate() {
    ...
    let _ = lfs_file_write(
        lfs.as_mut_ptr(),
        file.as_mut_ptr(),
        data1.as_ptr() as *const core::ffi::c_void,
        data1.len() as u32,
    );
    assert_ok(lfs_file_close(lfs.as_mut_ptr(), file.as_mut_ptr()));
    ...
    let _ = lfs_file_write(
        lfs.as_mut_ptr(),
        file.as_mut_ptr(),
        data2.as_ptr() as *const core::ffi::c_void,
        data2.len() as u32,
    );
    assert_ok(lfs_file_close(lfs.as_mut_ptr(), file.as_mut_ptr()));
    ...
    assert_eq!(lfs_file_size(lfs.as_mut_ptr(), file.as_mut_ptr()), 3);
    let mut buf = [0u8; 32];
    let n = lfs_file_read(
        lfs.as_mut_ptr(),
        file.as_mut_ptr(),
    ...
    assert_eq!(n, 3);
    assert_eq!(&buf[..3], data2);
    assert_ok(lfs_file_close(lfs.as_mut_ptr(), file.as_mut_ptr()));
    assert_ok(lfs_unmount(lfs.as_mut_ptr()));
}

// --- test_files_truncate_api ---
// Truncate via lfs_file_truncate API.
#[test]
