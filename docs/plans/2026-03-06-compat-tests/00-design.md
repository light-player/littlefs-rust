...
- Rename and refactor `lp-littlefs-c-align` into `lp-littlefs-compat`
- Rewrite to use the current `lp-littlefs` C-style API
- Port existing operation-level compat tests
- Add the 14 upstream `test_compat` forward/backward tests
- Reduce `lp-littlefs/tests/test_compat.rs` to the 3 version edge cases that belong there

## What stays in `lp-littlefs`

The 3 version edge cases (`major_incompat`, `minor_incompat`, `minor_bump`) test how `lp-littlefs` handles superblock version fields internally. They don't need a second implementation — they need internal APIs (`lfs_dir_fetch`, `lfs_dir_commit`, `LfsSuperblock`). These stay in `lp-littlefs/tests/test_compat.rs` and are covered by test-parity2 phase 7 (reduced to just those 3 cases).
...
- C function pointer callbacks for `lp_littlefs::LfsConfig` (same signature shape)
- Config builders for each side that wire up the callbacks and point at the shared storage

The `lp_littlefs::LfsConfig` and `littlefs2_sys::lfs_config` have compatible callback signatures (`unsafe extern "C" fn(...) -> i32`), but differ in struct layout (field types, field order). `SharedStorage` builds each separately from a common `TestGeometry` describing block_size, block_count, etc.
...
- `rust_impl`: Equivalent functions calling `lp_littlefs::lfs_*`. Same signatures, same semantics, different underlying implementation.
...
│   └── rust_impl.rs     # lp-littlefs wrappers
└── tests/
...
After phase 3, update test-parity2 phase 7 to cover only the 3 version edge cases in `lp-littlefs/tests/test_compat.rs`.
