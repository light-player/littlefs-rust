...
Port remaining tests incrementally; implement until all pass. Reuse helpers from littlefs-rust-old where useful.

## Tasks

1. **Port test utilities**
   - RAM block device, common setup/teardown
   - Helpers from `littlefs-rust-old/tests/common/mod.rs`
