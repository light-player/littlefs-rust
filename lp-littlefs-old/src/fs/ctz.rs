...
pub fn ctz_find<B: BlockDevice>(
    ...
    let mut current = ctz_index(ctx.config.block_size, &mut current_off);
    let target = ctz_index(ctx.config.block_size, &mut target_off);

    while current > target {
        let skip = (npw2(current - target + 1) - 1).min(ctz(current));
        let mut next_head = [0u8; 4];
        ctx.read(head, 4 * skip, &mut next_head)?;
...
pub fn ctz_extend<B: BlockDevice>(
    ...
    let nblock = alloc::alloc(ctx, root, lookahead)?;
    ctx.erase(nblock)?;
    ...
    let mut noff = size - 1;
    let index = ctz_index(ctx.config.block_size, &mut noff);
    let noff = noff + 1;

    if noff != block_size as u64 {
        for i in 0..noff {
            let mut data = [0u8; 1];
            ctx.read(head, i as u32, &mut data)?;
            ctx.prog(nblock, i as u32, &data)?;
        }
    ...
    for i in 0..skips {
        ...
        ctx.prog(nblock, 4 * i, &nhead_le)?;
        if i != skips - 1 {
            let mut buf = [0u8; 4];
            ctx.read(nhead, 4 * i, &mut buf)?;
    ...
    F: FnMut(u32) -> Result<(), Error>,
{
    ...
    let mut head = head;
    let mut current_off = size - 1;
    let mut index = ctz_index(ctx.config.block_size, &mut current_off);
    ...
    loop {
        ...
        let mut buf = [0u8; 8];
        ctx.read(head, 0, &mut buf[..count * 4])?;
...
mod tests {
    ...
    fn ctz_extend_empty_file() {
        ...
        let rcache = core::cell::RefCell::new(crate::fs::bdcache::new_read_cache(&config).unwrap());
        let pcache = core::cell::RefCell::new(crate::fs::bdcache::new_prog_cache(&config).unwrap());
        let ctx = BdContext::new(&bd, &config, &rcache, &pcache);
        let (block, off) = ctz_extend(&ctx, root, &mut lookahead, BLOCK_NULL, 0).unwrap();
        assert!(block >= 2);
        assert_eq!(off, 0);
    }
