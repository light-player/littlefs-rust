...
Cross-implementation compatibility tests that verify `littlefs-rust-core` (Rust) and the upstream C
...
original. Unit tests within `littlefs-rust-core` exercise the Rust code against itself, but cannot
...
and Rust (`littlefs_rust_core::LfsConfig`) configuration structs. A `TestGeometry` struct describes
...
- **`rust_impl`** — equivalent functions calling `littlefs_rust_core::lfs_*` with the same signatures
  and semantics.
...
- `littlefs-rust-core` — the Rust LittleFS implementation under test
