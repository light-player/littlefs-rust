...
| `log`          | no      | Logging via the `log` crate (`RUST_LOG=lp_littlefs_core=trace`)            |
...
```bash
cargo test -p lp-littlefs-core
cargo test -p lp-littlefs-core --features slow_tests  # includes power-loss tests
```
